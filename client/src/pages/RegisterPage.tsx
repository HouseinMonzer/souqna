...
function RegisterPage() {
  ...
  const validate = () => {
    if (!name.trim()) return isRTL ? 'الاسم مطلوب.' : 'Name is required.'
    if (name.trim().length < 2) return isRTL ? 'الاسم يجب أن يكون حرفين أو أكثر.' : 'Name must be at least 2 characters.'
    ...
    if (password.length < 8) return isRTL ? 'كلمة المرور يجب أن تكون 8 أحرف على الأقل.' : 'Password must be at least 8 characters.'
    if (password !== confirmPassword) return isRTL ? 'كلمتا المرور غير متطابقتين.' : 'Passwords do not match.'
    return ''
  }
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : (isRTL ? 'تعذّر إنشاء الحساب. حاول مجدداً.' : 'Unable to create your account. Please try again.'))
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <div style={{ backgroundColor: '#F7F2E8', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px', boxSizing: 'border-box' }} dir={isRTL ? 'rtl' : 'ltr'}>
      <form onSubmit={handleSubmit} noValidate style={{ width: '100%', maxWidth: 500, backgroundColor: '#fff', border: '1.5px solid #e0dbd0', borderRadius: 16, padding: '28px', boxSizing: 'border-box', boxShadow: '0 10px 34px rgba(45,74,30,0.08)' }}>
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          ...
          <h1 style={{ fontFamily: 'Georgia, serif', fontSize: 24, color: '#1A2E0E', margin: '18px 0 6px' }}>{t('auth.registerTitle')}</h1>
          <p style={{ color: '#7a8a6e', fontSize: 14, margin: 0 }}>{t('auth.registerSubtitle')}</p>
        </div>
        ...
        <label style={{ display: 'block', fontSize: 13, fontWeight: 700, color: '#1A2E0E', marginBottom: 6 }} htmlFor="name">{t('auth.fullName')}</label>
        ...
        <label style={{ display: 'block', fontSize: 13, fontWeight: 700, color: '#1A2E0E', marginBottom: 6 }} htmlFor="confirm-password">{isRTL ? 'تأكيد كلمة المرور' : 'Confirm password'}</label>
        ...
        <p style={{ textAlign: 'center', color: '#7a8a6e', fontSize: 14, margin: '20px 0 0' }}>
          {t('auth.haveAccount')} <Link to="/sign-in" style={{ color: '#5C8A2E', fontWeight: 700, textDecoration: 'none' }}>{t('auth.signIn')}</Link>
        </p>
