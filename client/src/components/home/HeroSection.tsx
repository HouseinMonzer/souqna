...
function HeroSection() {
  ...
  return (
    <section style={{ backgroundColor: '#2D4A1E', padding: '56px 24px', position: 'relative', overflow: 'hidden' }} dir={isRTL ? 'rtl' : 'ltr'}>
      <div style={{ position: 'absolute', [isRTL ? 'left' : 'right']: '-40px', top: '-40px', width: '280px', height: '280px', borderRadius: '50%', backgroundColor: 'rgba(92,138,46,0.08)', pointerEvents: 'none' } as React.CSSProperties} />
      <div style={{ position: 'absolute', [isRTL ? 'left' : 'right']: '80px', top: '20px', width: '160px', height: '160px', borderRadius: '50%', backgroundColor: 'rgba(92,138,46,0.06)', pointerEvents: 'none' } as React.CSSProperties} />
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '32px', flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1, minWidth: '260px' }}>
          ...
          <h1 style={{ color: '#F7F2E8', fontFamily: 'Georgia, serif', fontSize: 'clamp(28px, 5vw, 52px)', fontWeight: '700', lineHeight: '1.15', marginBottom: '16px' }}>
            {t('hero.title1')}<br />{t('hero.title2')}{' '}
            <span style={{ color: '#A3C46C', fontStyle: 'italic' }}>{t('hero.titleEmph')}</span>
          </h1>
        ...
        <div style={{ backgroundColor: 'rgba(92,138,46,0.15)', border: '1px solid rgba(163,196,108,0.4)', borderRadius: '16px', padding: '28px 36px', textAlign: 'center', flexShrink: 0 }}>
          ...
          <div style={{ color: 'rgba(247,242,232,0.65)', fontSize: '14px', marginTop: '4px' }}>{t('hero.activeVendors')}</div>
        </div>
